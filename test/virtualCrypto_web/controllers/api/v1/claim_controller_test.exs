defmodule ClaimControllerTest do
  ...
  test "get user1 claims with invalid token by user1", %{conn: conn} = ctx do
    ...
    conn = get(conn, Routes.claim_path(conn, :me))
  ...
  test "get user1 claim0", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.claim_path(conn, :get_by_id, (claims |> at(0) |> elem(0)).id))
  ...
  test "get claim1 by payer", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.claim_path(conn, :get_by_id, (claims |> at(1) |> elem(0)).id))
  ...
  test "get claim2 by claimant", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.claim_path(conn, :get_by_id, (claims |> at(2) |> elem(0)).id))
  ...
  test "get claim3 by claimant", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.claim_path(conn, :get_by_id, (claims |> at(3) |> elem(0)).id))
  ...
  test "get claim4 by claimant", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.claim_path(conn, :get_by_id, (claims |> at(4) |> elem(0)).id))
  ...
  test "get claim by payer and claimant", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.claim_path(conn, :get_by_id, (claims |> at(5) |> elem(0)).id))
  ...
  test "patch claim0 with nothing status", %{conn: conn, claims: claims, user1: user1} do
    ...
    conn = patch(conn, Routes.claim_path(conn, :patch, (claims |> at(0) |> elem(0)).id))
  ...
  test "patch claim0 with invalid status", %{conn: conn, claims: claims, user1: user1} do
    ...
    conn =
      patch(conn, Routes.claim_path(conn, :patch, (claims |> at(0) |> elem(0)).id), %{
        "status" => "nyan!"
      })
  ...
  test "approve claim by claimant", %{conn: conn, claims: claims, user1: user1} do
    ...
    conn =
      patch(
        conn,
        Routes.claim_path(conn, :patch, (claims |> at(0) |> elem(0)).id),
        %{"status" => "approved"}
      )
  ...
  test "approve claim by payer but not_enough_amount",
    ...
    conn =
      patch(
        conn,
        Routes.claim_path(conn, :patch, (claims |> at(1) |> elem(0)).id),
        %{"status" => "approved"}
      )
