...
payViewSubmitValidation model =
    if String.toInt model.amount == Maybe.Nothing then
        False

    else if String.toInt model.user_id == Maybe.Nothing then
        False

    else if model.unit == "" then
        False
...
payViewSubmitButton model =
    if payViewSubmitValidation model then
        button [ class "button has-background-info has-text-white", onClick Submit ] [ text "送金" ]

    else
        button [ class "button has-background-info has-text-white", attribute "disabled" "true" ] [ text "送金" ]
    ...
    div [ class "dropdown is-hoverable" ]
        [ div [ class "dropdown-trigger" ]
            [ p [ attribute "aria-haspopup" "true", attribute "aria-controls" "dropdown-menu" ]
                [ span [ class "has-text-info has-text-weight-bold" ] [ text <| "@" ++ u.discord.username ]
                ]
            ]
        , div [ class "dropdown-menu", id "dropdown-menu", attribute "role" "menu" ]
            [ div [ class "dropdown-content" ]
                ...
                , div [ class "dropdown-item" ]
                    [ p [ class "has-text-weight-bold has-text-centered is-size-4" ] [ text <| u.discord.username ]
                    ]
