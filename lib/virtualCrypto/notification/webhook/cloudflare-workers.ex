if VirtualCrypto.Notification.Webhook.CloudflareWorkers in Keyword.get(
     Application.compile_env!(:virtualCrypto, VirtualCrypto.Notification.Dispatcher),
     :children,
     []
   ) do
  defmodule VirtualCrypto.Notification.Webhook.CloudflareWorkers do
    ...
    def config, do: Application.fetch_env!(:virtualCrypto, __MODULE__)
      ...
      case execute_raw(
        ...
        {:ok, res} ->
          if res.status_code != 200 do
            Logger.warning("proxy respond with #{res.status_code}")
          end

          case res.headers |> List.keyfind("X-Status", 0) do
            nil ->
              Logger.warning("missing X-Status header: #{res.body}")
            ...
            {_, v} ->
              case Integer.parse(v) do
                ...
                _ ->
                  Logger.warning("invalid X-Status header value")
        ...
        {:error, err} ->
          Logger.warning("an error occurred while processing verification", err)
          :error
      end
    end
